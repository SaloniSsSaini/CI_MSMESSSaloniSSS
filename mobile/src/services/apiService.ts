...
// const API_BASE_URL = 'http://192.168.29.54:5000/api'; // Change this to your backend URL
...
// const API_BASE_URL = 'http://localhost:5000/api'; // Change this to your backend URL
// const API_BASE_URL = 'http://192.168.29.54:5000/api'; // Change this to your backend URL
// const API_BASE_URL = 'http://192.168.0.107:5000/api'; // Change this to your backend URL
const API_BASE_URL = 'http://3.109.203.124:5000/api'; // Change this to your backend URL
...
class ApiService {
  ...
  async getSMSTransactions(params?: Record<string, string>) {
    const queryString = params ? `?${new URLSearchParams(params).toString()}` : '';
    return this.makeRequest('GET', `/sms/transactions${queryString}`);
  }

  async getSMSAnalytics(params?: Record<string, string>) {
    const queryString = params ? `?${new URLSearchParams(params).toString()}` : '';
    return this.makeRequest('GET', `/sms/analytics${queryString}`);
  }
  ...
  async getEmailTransactions(params?: Record<string, string>) {
    const queryString = params ? `?${new URLSearchParams(params).toString()}` : '';
    return this.makeRequest('GET', `/email/transactions${queryString}`);
  }

  async getEmailAnalytics(params?: Record<string, string>) {
    const queryString = params ? `?${new URLSearchParams(params).toString()}` : '';
    return this.makeRequest('GET', `/email/analytics${queryString}`);
  }
  ...
  async getCarbonAssessments(params?: Record<string, string>) {
    const queryString = params ? `?${new URLSearchParams(params).toString()}` : '';
    return this.makeRequest('GET', `/carbon/assessments${queryString}`);
  }
  ...
  async getAnalyticsTrends(params?: Record<string, string>) {
    const queryString = params ? `?${new URLSearchParams(params).toString()}` : '';
    return this.makeRequest('GET', `/analytics/trends${queryString}`);
  }

  async getAnalyticsInsights(params?: Record<string, string>) {
    const queryString = params ? `?${new URLSearchParams(params).toString()}` : '';
    return this.makeRequest('GET', `/analytics/insights${queryString}`);
  }
  ...
  async getCarbonOffsetOptions(params?: Record<string, string>) {
    const queryString = params ? `?${new URLSearchParams(params).toString()}` : '';
    return this.makeRequest('GET', `/carbon/trading/offsets${queryString}`);
  }
  ...
  async getCarbonTradingHistory(params?: Record<string, string>) {
    const queryString = params ? `?${new URLSearchParams(params).toString()}` : '';
    return this.makeRequest('GET', `/carbon/trading/history${queryString}`);
  }

  async getCarbonMarketTrends(params?: Record<string, string>) {
    const queryString = params ? `?${new URLSearchParams(params).toString()}` : '';
    return this.makeRequest('GET', `/carbon/trading/market-trends${queryString}`);
  }
  ...
  async setToken(token: string): Promise<void> {
    this.token = token;
    await AsyncStorage.setItem('userToken', token);
    // Update the tokenInitialized promise to be resolved
    this.tokenInitialized = Promise.resolve();
  }
